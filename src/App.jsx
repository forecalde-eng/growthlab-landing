...
function useMatrixRain(canvasRef) {
  useEffect(() => {
    ...
    function draw() {
      ctx.fillStyle = 'rgba(15,20,35,0.05)';
      ctx.fillRect(0, 0, canvas.width, canvas.height);
      ctx.font = `${FONT_SIZE}px monospace`;
      for (let i = 0; i < cols; i++) {
        const char = MATRIX_CHARS[Math.floor(Math.random() * MATRIX_CHARS.length)];
        // Brand gradient: left columns cyan, right columns magenta
        const t = i / cols;
        ctx.fillStyle = t < 0.5 ? '#48BED7' : '#C057D5';
...
/* ─── Brand: Flask Logo (Erlenmeyer + growth arrow + G) ─── */
function FlaskLogo({ size = 28 }) {
  const gid = `gl-grad-${size}`;
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none" style={{ display: "block", flexShrink: 0 }}>
      <defs>
        <linearGradient id={gid} x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#48BED7" />
          <stop offset="100%" stopColor="#C057D5" />
        </linearGradient>
      </defs>
      {/* Flask lip */}
      <path d="M22 8 L42 8" stroke={`url(#${gid})`} strokeWidth="3.5" strokeLinecap="round" />
      {/* Flask neck (left) */}
      <path d="M24 8 L24 22" stroke={`url(#${gid})`} strokeWidth="3.5" strokeLinecap="round" />
      {/* Right neck → growth arrow */}
      <path d="M40 8 L40 18 L52 6" stroke={`url(#${gid})`} strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M45 6 L52 6 L52 13" stroke={`url(#${gid})`} strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" />
      {/* Flask body (trapezoid w/ rounded bottom) */}
      <path d="M24 22 L10 50 Q10 58 18 58 L46 58 Q54 58 54 50 L40 22"
            stroke={`url(#${gid})`} strokeWidth="3.5" strokeLinejoin="round" strokeLinecap="round" />
      {/* G letter inside body */}
      <path d="M38 36 A8 8 0 1 0 38 46 L38 41 L34 41"
            stroke={`url(#${gid})`} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" fill="none" />
    </svg>
  );
}
...
function Wordmark({ size = 20 }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 10 }}>
      <FlaskLogo size={size * 1.5} />
      <span style={{ fontFamily: "Montserrat, sans-serif", fontSize: size, fontWeight: 700, letterSpacing: "-0.01em", lineHeight: 1 }}>
        GrowthLab{" "}
        <span style={{
          background: "linear-gradient(135deg, #48BED7, #C057D5)",
  ...
  return (
    <div style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
      <button onClick={() => setOpen(!open)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 0", background: "none", border: "none", cursor: "pointer", textAlign: "left" }}>
        <span style={{ fontFamily: "Montserrat, sans-serif", fontSize: 15, fontWeight: 500, color: "#F0F2F8", paddingRight: 16 }}>{faq.q}</span>
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: bg, color: txt, fontFamily: "Inter, sans-serif", WebkitFontSmoothing: "antialiased" }}>
      {/* NAV */}
      <nav style={{ position: "sticky", top: 0, zIndex: 50, borderBottom: scrolled ? `1px solid ${brd}` : "1px solid transparent", background: scrolled ? "rgba(15,20,35,0.9)" : "transparent", backdropFilter: scrolled ? "blur(20px)" : "none", transition: "all 0.4s ease" }}>
      ...
      <section style={{ position: "relative", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", overflow: "hidden", background: bg, padding: "80px 24px" }}>
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 1, background: "radial-gradient(ellipse 70% 60% at 50% 50%, rgba(15,20,35,0.5) 0%, rgba(15,20,35,0.88) 100%)" }} />
        {/* Content */}
        <div style={{ position: "relative", zIndex: 10, textAlign: "center", maxWidth: 720, width: "100%" }}>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            {/* Scramble title */}
            <h1 style={{ fontFamily: "Montserrat, sans-serif", fontSize: "clamp(2rem,5vw,3.5rem)", fontWeight: 700, lineHeight: 1.2, marginBottom: 24, color: txt, minHeight: "2.5em" }}>
              {scrambledText}
            </h1>
            {/* Subtitle */}
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: "rgba(72,190,215,0.15)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: teal }}>AI</div>
            ...
            {chatMsgs.map((m, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: m.from === "bot" ? -10 : 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 + i * 0.3 }}
                ...
                <span style={{ fontSize: 13, padding: "8px 12px", borderRadius: 10, maxWidth: "80%", background: m.from === "bot" ? "rgba(255,255,255,0.05)" : `rgba(72,190,215,0.15)`, color: m.from === "bot" ? muted : teal, lineHeight: 1.4 }}>{m.text}</span>
              </motion.div>
            ))}
      ...
      <section id="servicios" ref={probRef} style={{ padding: "80px 24px", borderTop: `1px solid ${brd}` }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.4 }} style={{ marginBottom: 40 }}>
            ...
            <h2 style={{ fontFamily: "Montserrat, sans-serif", fontSize: "clamp(26px, 4vw, 42px)", fontWeight: 600 }}>Tu negocio pierde dinero <span style={{ color: coral }}>de estas formas</span></h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
            {problems.map((p, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.35, delay: i * 0.1 }}
                ...
                <p style={{ fontFamily: "Montserrat", fontSize: "clamp(36px,5vw,52px)", fontWeight: 700, color: coral, marginBottom: 8, lineHeight: 1 }}>{p.stat}</p>
                <h3 style={{ fontFamily: "Montserrat", fontSize: 16, fontWeight: 500, marginBottom: 8 }}>{p.title}</h3>
      ...
      {/* SOLUTION */}
      <section style={{ padding: "80px 24px", borderTop: `1px solid ${brd}`, background: "radial-gradient(ellipse 700px 500px at 50% 50%, rgba(192,87,213,0.04), transparent)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.4 }} style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <h2 style={{ fontFamily: "Montserrat, sans-serif", fontSize: "clamp(26px, 4vw, 38px)", fontWeight: 600, marginBottom: 10 }}>Dos fuerzas, <span style={{ color: purple }}>un solo equipo</span></h2>
            ...
            {[{ label: "T", title: "Growth & Customer Service", sub: "El lado humano", items: pillarT, accent: purple },
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.4, delay: i * 0.15 }}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: `rgba(${i===0?"192,87,213":"72,190,215"},0.15)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: col.accent }}>{col.label}</div>
                  <div><p style={{ fontFamily: "Montserrat", fontSize: 15, fontWeight: 500 }}>{col.title}</p><p style={{ fontSize: 11, color: col.accent }}>{col.sub}</p></div>
                </div>
          ...
          <div style={{ textAlign: "center", marginTop: 24, display: "flex", justifyContent: "center", gap: 8, alignItems: "center" }}>
            {[["Estrategia", purple], ["Tecnología", teal], ["Resultados reales", amber]].map(([label, color], i) => (
              <span key={i} style={{ fontSize: 12, color, padding: "4px 14px", borderRadius: 20, background: `rgba(${color === purple ? "192,87,213" : color === teal ? "72,190,215" : "72,190,215"},0.08)`, border: `1px solid ${color}30` }}>{label}{i < 2 ? " +" : ""}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section id="proceso" style={{ padding: "72px 24px", borderTop: `1px solid ${brd}`, backgroundImage: "linear-gradient(rgba(255,255,255,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.02) 1px, transparent 1px)", backgroundSize: "60px 60px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.4 }} style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <h2 style={{ fontFamily: "Montserrat, sans-serif", fontSize: "clamp(26px, 4vw, 38px)", fontWeight: 600, marginBottom: 10 }}>De cero a resultados en 3 pasos</h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 20 }}>
            {steps.map((s, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.35, delay: i * 0.12 }}
                ...
                <div style={{ width: 50, height: 50, borderRadius: "50%", background: "rgba(72,190,215,0.1)", border: "1px solid rgba(72,190,215,0.2)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontFamily: "Montserrat", fontSize: 20, fontWeight: 600, color: amber }}>{s.num}</div>
                <h3 style={{ fontFamily: "Montserrat", fontSize: 16, fontWeight: 500, marginBottom: 8 }}>{s.title}</h3>
      ...
      <section id="faq" style={{ padding: "72px 24px", borderTop: `1px solid ${brd}` }}>
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.4 }} style={{ textAlign: "center", marginBottom: 40 }}>
            ...
            <h2 style={{ fontFamily: "Montserrat, sans-serif", fontSize: "clamp(26px, 4vw, 38px)", fontWeight: 600 }}>Preguntas frecuentes</h2>
          </motion.div>
      ...
      {/* CTA FINAL */}
      <section style={{ padding: "80px 24px", borderTop: `1px solid ${brd}`, textAlign: "center", background: "radial-gradient(ellipse 600px 400px at 50% 0%, rgba(72,190,215,0.06), transparent)" }}>
        <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.4 }} style={{ maxWidth: 520, margin: "0 auto" }}>
          <h2 style={{ fontFamily: "Montserrat, sans-serif", fontSize: "clamp(28px, 5vw, 44px)", fontWeight: 600, marginBottom: 12, lineHeight: 1.15 }}>¿Listo para crecer con IA?</h2>
